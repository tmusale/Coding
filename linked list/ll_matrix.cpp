...
NULL NULL NULL
Input:
The fuction takes 2 argument as input, first the 2D matrix mat[][] and second an integer variable N, 
...
should be used by a user for Expected Output only. 
...
Node * make_matrix(int mat[MAX][MAX], int n, int i, int j)
{
    if (i >= n || j >= n) {
        return NULL;
    }
    ...
    newnode->right = make_matrix(mat, n, i, j+1);
    newnode->down = make_matrix(mat, n, i+1, j);
