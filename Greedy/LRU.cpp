...
to decide which page needs to be replaced when the new page comes in. Whenever a new page is referred 
and is not present in memory, the page fault occurs and Operating System replaces one of the existing 
pages with a newly needed page. Given a sequence of pages and memory capacity, your task is to find the 
...
n number of pages and next line contains space seaprated sequence of pages. The following line consist of 
...
bool minbysec(const pair<int, int> &a, const pair<int, int> &b)
{
	    ...
	    for (int i = 0; i < n; i++) {
	        if (mem.size() < c) {
	            ...
	            if (mem.find(arr[i]) != mem.end()) {
	                ...
	                auto it = std::min_element(index.begin(), index.end(), minbysec);
	                int temp = it->first;
	                mem.erase(temp);
	                index.erase(temp);
	                mem.insert(arr[i]);
